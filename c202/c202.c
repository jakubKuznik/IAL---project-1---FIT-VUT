...
void Stack_Init( Stack *stack ) {
	if (!stack){   //If stack == NULL call error
		Stack_Error(SERR_INIT);
		return;
	}
	stack->topIndex = -1; //Set stack to empty.
}
...
int Stack_IsEmpty( const Stack *stack ) {
	...
	// If stack has positive size substract absolute value of size always get 0.
	// So if tha stack is empty we ll get -2 value
	return stack->topIndex - (stack->topIndex * ((stack->topIndex > 0) - (stack->topIndex < 0)));
}
...
void Stack_Pop( Stack *stack ) {
	...
	stack->topIndex--; // Move pointer to stack one position lower. Doesn't clean data.
}
